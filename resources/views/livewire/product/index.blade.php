<div>
    <x-header title="Produtos" separator progress-indicator>
        ...
        <x-slot:actions>
            ...
            <x-button label="Adicionar" class="btn-primary" link="{{ route('admin.product.create') }}"
                icon="o-plus-circle" />
        </x-slot:actions>
    </x-header>
        ...
        <x-table :headers="$headers" :rows="$this->rows" :sort-by="$sortBy" with-pagination per-page="perPage"
            ...
            @scope('actions', $product)
                <div class="flex gap-3">
                    <x-button icon="o-pencil-square" link="{{ route('admin.product.edit', $product['id']) }}" spinner
